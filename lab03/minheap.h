...
// Heap class
template <typename E, typename Comp> class minheap {
private:
	E* Heap;          // Pointer to the heap array 堆对应的数列
	...
	void siftdown(int pos) {
		while (!isLeaf(pos)) { // Stop if pos is a leaf
			...
			if ((rc < n) && Comp::prior(Heap[rc], Heap[j]))//第一个参数应该排在第二个参数之前 返回true
				j = rc;            // Set j to greater child's value
			if (Comp::prior(Heap[pos], Heap[j])) return; // Done父节点的值比子节点的最大值小 
			swap(Heap, pos, j);//互换
			pos = j;             // Move down
		}
	}
...
public:
	minheap(E* h, int num, int max)     // Constructor
	{
		Heap = h;  n = num;  maxsize = max;  buildMinHeap();
	}
	...
	int parent(int pos) const  // Return parent position  返回父节点
	{
		return (pos - 1) / 2;
	}
	void buildMinHeap()           // Heapify contents of Heap
	{
